...
import (
	...
	"github.com/selimk92/percentage-resource-operator/test/utils"
)

// namespace where the project is deployed in
const namespace = "percentage-system"

// serviceAccountName created for the project
const serviceAccountName = "percentage-controller-manager"

// metricsServiceName is the name of the metrics service of the project
const metricsServiceName = "percentage-controller-manager-metrics-service"

// metricsRoleBindingName is the name of the RBAC that will be created to allow get the metrics data
const metricsRoleBindingName = "percentage-metrics-binding"
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				"--clusterrole=percentage-metrics-reader",
			...
			verifyWebhookEndpointsReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "endpointslices.discovery.k8s.io", "-n", namespace,
					"-l", "kubernetes.io/service-name=percentage-webhook-service",
			...
			verifyMutatingWebhookReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "mutatingwebhookconfigurations.admissionregistration.k8s.io",
					"percentage-mutating-webhook-configuration",
			...
			verifyValidatingWebhookReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "validatingwebhookconfigurations.admissionregistration.k8s.io",
					"percentage-validating-webhook-configuration",
					"-o", "jsonpath={.webhooks[0].clientConfig.caBundle}")
				output, err := utils.Run(cmd)
